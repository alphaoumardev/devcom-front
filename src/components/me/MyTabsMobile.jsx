...
const MyTabsMobile = ({following, liked_posts, saved_posts, followedby})=>
{
    const {my_info} = useSelector(state=> state.getMyInfoReducer)

    const tabs = {
        myPosts: <div className={"flex justify-center items-center space-x-1 text-gray-900"}><FaBlogger size={25}/> <span className="text-gray-900 sm:text-xl text-sm">{my_info?.my_posts_count}</span></div>,
        flow: <div className={"flex justify-center items-center space-x-1 text-gray-900"}><GiShadowFollower size={25}/> <span className="text-gray-900 sm:text-xl text-sm">{my_info?.followedby_count}</span></div>,
        foll: <div className={"flex justify-center items-center space-x-1 text-gray-900"}> <BsPeople size={25}/> <span className="text-gray-900 sm:text-xl text-sm">{my_info?.follow_count}</span></div>,
