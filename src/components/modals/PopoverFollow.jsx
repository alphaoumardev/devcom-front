...
const PopoverFollow = ({item, my_profile})=>
{
        ...
        <div className=" w-56 h-40 pb-3 transition-opacity duration-300 dark:text-gray-400 dark:bg-gray-800 dark:border-gray-600">
            <div className="flex justify-between items-center mb-2">
                <a href={`/hisprofile/${item?.id}`}>
                    {item?.avatar?
                        <div>
                            <img  src={item?.avatar} alt="" className="hidden object-cover w-10 h-10 mx-4 rounded-full sm:block" />
                        </div>:
                        <div>
            ...
            <p className="mb-4 text-sm font-light">{item?.bio?.slice(0,40)} ...
